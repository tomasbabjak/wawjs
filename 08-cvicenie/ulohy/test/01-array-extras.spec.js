...
describe("FP - array functions", function() {
  ...
  it("02-students without git repo", () => {
    ...
    assert.deepStrictEqual(missingGit.map(({ name }) => name), [
      "Student 2",
      "Student 26"
    ]);
  });
  ...
  it("04-find students with less then 3 projects", () => {
    ...
    assert.deepStrictEqual(students4.map(s => s["#"]), [
      6,
      7,
      12,
      21,
...
>>>>>>> upstream/master
  ...
  it("06-total points of each student", () => {
    ...
    let studentsWithPoints = students.map(fixPoints).map(totalPoints);
...
<<<<<<< HEAD
    // za vsetkych
    let sumOfAll = students.map(fixPoints).map(totalPoints);
=======
    // za vsetkych 
...
<<<<<<< HEAD
    ...
    uniqueProjects.map(uniqueProjects => [{ key: uniqueProjects, value: [] }]);
